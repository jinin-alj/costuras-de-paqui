...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for name, order, desc in stages_data:
            ...
            stages[name] = s
        self.stdout.write(self.style.SUCCESS(f'  ✓ {len(stages)} production stages'))
        ...
        for fn, ln, role, phone, specialty in employees_data:
            ...
            if not e.user:
                ...
                e.save()
            emp_objs.append(e)
        self.stdout.write(self.style.SUCCESS(f'  ✓ {len(emp_objs)} employees (password: staff123)'))
        ...
        for fn, ln, phone, email, notes in customers_data:
            ...
            cust_objs.append(c)
        self.stdout.write(self.style.SUCCESS(f'  ✓ {len(cust_objs)} customers'))
        ...
        if not CustomerOrder.objects.filter(customer=cust_objs[0]).exists():
            ...
            self.stdout.write(self.style.SUCCESS('  ✓ Order 1: Delivered bridal gown'))
        ...
        if not CustomerOrder.objects.filter(customer=cust_objs[1]).exists():
            ...
            self.stdout.write(self.style.SUCCESS('  ✓ Order 2: In-production suit'))
        ...
        if not CustomerOrder.objects.filter(customer=cust_objs[2]).exists():
            ...
            self.stdout.write(self.style.SUCCESS('  ✓ Order 3: OVERDUE urgent gown'))
        ...
        if not CustomerOrder.objects.filter(customer=cust_objs[3]).exists():
            ...
            self.stdout.write(self.style.SUCCESS('  ✓ Order 4: New received order'))
